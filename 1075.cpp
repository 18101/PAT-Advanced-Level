...
int main()
{
	...
	int rank=1;
	printf("%d %05d %d",rank,sb[1].id,sb[1].total);
	for(int j=1;j<=k;j++)
	{
		if(pro[sb[1].id][j])
		{
			printf(" %d",pro[sb[1].id][j]);
		}
		else
		{
			if(up[sb[1].id][j])
	...
	{
		if(sb[i].total!=sb[i-1].total)
			rank=i;
		printf("%d %05d %d",rank,sb[i].id,sb[i].total);
		for(int j=1;j<=k;j++)
		{
			if(pro[sb[i].id][j])
			{
				printf(" %d",pro[sb[i].id][j]);
			}
			else
			{
				if(up[sb[i].id][j])
